...
Features:
  * Overview: CPU, memory, storage, uptime, hostname rename.
...
The first run creates a random web password in the data directory. By default:
  /var/lib/serverdeck/admin-password.txt

Security note: this app uses HTTP Basic authentication but not TLS. Keep it on a
trusted LAN or behind a VPN/reverse proxy. Do not expose it directly to the web.
"""
...
def update_hosts_file(old_hostname: str, new_hostname: str) -> None:
    ...
    for index, line in enumerate(lines):
        ...
        if fields and fields[0] == "127.0.1.1":
            aliases = fields[1:]
            if old_hostname in aliases or not aliases:
                aliases = [new_hostname if item == old_hostname else item for item in aliases]
                if new_hostname not in aliases:
                    aliases.insert(0, new_hostname)
                lines[index] = "127.0.1.1\t" + " ".join(dict.fromkeys(aliases))
                changed = True
            break
    if changed:
...
def nav(active: str) -> str:
    items = [("overview", "/", "Overview"), ("updates", "/updates", "System updates"), ("backup", "/backup", "Backup")]
...
def page(title: str, active: str, body: str, script: str, csrf_token: str) -> str:
    return f"""<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<meta name="csrf-token" content="{html.escape(csrf_token)}"><title>{html.escape(title)} · {APP_NAME}</title><style>{CSS}</style></head>
<body><header class="topbar"><div class="topbar-inner"><div class="brand"><span class="brand-mark">SD</span><span>{APP_NAME}</span></div>{nav(active)}</div></header>
<div class="shell"><main>{body}</main><div class="footer">{APP_NAME} {APP_VERSION} · Keep this service on a trusted LAN or behind a VPN.</div></div>
...
loadStats(); setInterval(loadStats,2500);
"""
...
<div class="grid" style="margin-top:18px">
  <article class="card span-5"><div class="section-title"><h2 id="form-title">New backup job</h2><button class="button ghost hidden" id="cancel-edit">Cancel edit</button></div>
    <form id="backup-form" class="stack">
      ...
      <div class="field"><label for="job-source">Source</label><input class="input mono" id="job-source" required placeholder="/srv/photos/"><div class="help">A trailing slash copies the contents of a folder rather than the folder itself.</div></div>
      <div class="field"><label for="job-destination">Destination</label><input class="input mono" id="job-destination" required placeholder="/mnt/backup/photos/"><div class="help">Remote rsync destinations such as <span class="mono">user@server:/path/</span> are also accepted.</div></div>
...
class Application:
    ...
    def auth_ok(self, header: str) -> bool:
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def authenticate(self) -> bool:
        if self.app.auth_ok(self.headers.get("Authorization", "")):
            return True
    ...
    def require_csrf(self) -> bool:
        provided = self.headers.get("X-ServerDeck-Token", "")
        if hmac.compare_digest(provided, self.app.csrf_token):
            return True
    ...
    def do_GET(self) -> None:
        ...
        try:
            if path == "/":
                self.send_html(page("Overview", "overview", OVERVIEW_BODY, OVERVIEW_JS, self.app.csrf_token))
            elif path == "/updates":
                self.send_html(page("System updates", "updates", UPDATES_BODY, UPDATES_JS, self.app.csrf_token))
            elif path == "/backup":
                self.send_html(page("Backup", "backup", BACKUP_BODY, BACKUP_JS, self.app.csrf_token))
            elif path == "/api/stats":
    ...
    def do_PUT(self) -> None:
        ...
        match = re.fullmatch(r"/api/backups/([A-Za-z0-9_-]+)", path)
        if not match:
            self.error_json(404, "Not found")
            return
        try:
...
[Service]
Type=simple
ExecStart=/usr/bin/python3 {systemd_quote(str(target))} --host {systemd_quote(args.host)} --port {args.port} --data-dir {systemd_quote(str(data_dir))} --username {systemd_quote(args.username)}
WorkingDirectory={systemd_quote(str(install_dir))}
Restart=on-failure
RestartSec=3
User=root
UMask=0077
...
"""
    ...
    if result.returncode == 0:
        result = run_command(["systemctl", "enable", "--now", "serverdeck.service"], timeout=45)
    if result.returncode != 0:
        print(result.stdout, file=sys.stderr)
        return result.returncode or 1
    password_file = data_dir / "admin-password.txt"
    for _ in range(30):
    ...
    if password_file.exists():
        print(f"Password: {password_file.read_text(encoding='utf-8').strip()}")
    else:
        print(f"Read the password with: sudo cat {password_file}")
    print("Keep the service on a trusted LAN or behind a VPN; HTTP Basic authentication is not encrypted without HTTPS.")
    return 0
...
def uninstall_service() -> int:
    ...
    print("ServerDeck service removed. Backup job data was left in place.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--username", default="admin", help="HTTP Basic Auth username (default: admin)")
    parser.add_argument("--password", help="HTTP password; prefer SERVERDECK_PASSWORD or --password-file")
    parser.add_argument("--password-file", help="File containing the HTTP password")
    parser.add_argument("--no-auth", action="store_true", help="Disable authentication (trusted testing networks only)")
...
def main() -> int:
    ...
    if not config.is_root:
        print("Running without root: hostname changes, package operations, and system schedules will be unavailable.")
