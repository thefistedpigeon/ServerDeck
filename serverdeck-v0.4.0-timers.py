...
Features:
  ...
  * Timers: run Python scripts or commands at boot or on recurring systemd schedules.
...
APP_NAME = "ServerDeck"
APP_VERSION = "0.4.0"
APP_BUILD = "2026-07-30-system-timers"
DEFAULT_PORT = 8081
MAX_BODY = 1024 * 1024
...
class Config:
    def __init__(self, args: argparse.Namespace):
        ...
        self.timers_file = self.data_dir / "timer-jobs.json"
        ...
        if self.auth_mode == "pam" and not args.run_backup and not args.run_timer:
            ensure_pam_config()
...
class TimerStore:
    """Persistent definitions for boot-time and recurring command jobs."""

    ALLOWED_KINDS = {"python", "command"}
    ALLOWED_TRIGGERS = {"boot", "hourly", "daily", "weekly", "monthly", "custom"}
    ...
    def validate(cls, payload: Dict[str, Any], timer_id: Optional[str] = None) -> Dict[str, Any]:
        if not isinstance(payload, dict):
            raise ValueError("Invalid timer job")
        name = safe_text(payload.get("name", ""), 80)
        if not JOB_NAME_RE.match(name):
            raise ValueError("Enter a timer name between 1 and 80 characters")
        ...
        trigger = safe_text(schedule_raw.get("trigger", "daily"), 20).lower()
        expression = safe_text(schedule_raw.get("expression", ""), 200)
        persistent = bool(schedule_raw.get("persistent", True))
        try:
            boot_delay = int(schedule_raw.get("boot_delay", 30))
        except (TypeError, ValueError) as exc:
            raise ValueError("Boot delay must be a whole number of seconds") from exc
        if trigger not in cls.ALLOWED_TRIGGERS:
            raise ValueError("Invalid timer schedule")
        if not 0 <= boot_delay <= 3600:
            raise ValueError("Boot delay must be between 0 and 3600 seconds")
        if trigger == "custom":
            if not expression or any(ch in expression for ch in "\r\n\x00"):
                raise ValueError("Enter a valid systemd OnCalendar expression")
            if command_exists("systemd-analyze"):
                checked = run_command(["systemd-analyze", "calendar", "--", expression], timeout=10)
                if checked.returncode != 0:
                    raise ValueError(checked.stdout.strip() or "Enter a valid systemd OnCalendar expression")
        else:
            expression = ""
...
def timer_calendar(job: Dict[str, Any]) -> str:
    ...
    if trigger == "custom":
        value = schedule.get("expression", "").strip()
        if not value or any(ch in value for ch in "\r\n\x00"):
            raise ValueError("Invalid systemd OnCalendar expression")
        return value
    if trigger == "boot":
        raise ValueError("Boot jobs do not use OnCalendar")
...
class TimerSchedulerManager:
    """Install managed systemd services/timers for general scheduled tasks."""
    ...
    def _service_text(self, job: Dict[str, Any]) -> str:
        ...
        lines = [
            "[Unit]",
            f"Description=ServerDeck task: {job['name']}",
        ...
        if schedule.get("trigger") == "boot" and int(schedule.get("boot_delay", 0)) > 0:
            lines.append(f"ExecStartPre=/bin/sleep {int(schedule['boot_delay'])}")
        lines.extend(
            [
        ...
        if schedule.get("trigger") == "boot":
            lines.extend(["[Install]", "WantedBy=multi-user.target", ""])
        return "\n".join(lines)

    @staticmethod
    def _timer_text(job: Dict[str, Any]) -> str:
        schedule = job.get("schedule", {})
        lines = [
            "[Unit]",
            f"Description=Schedule ServerDeck task: {job['name']}",
            "",
            "[Timer]",
            f"OnCalendar={timer_calendar(job)}",
            f"Persistent={'true' if schedule.get('persistent', True) else 'false'}",
            f"Unit=serverdeck-task-{job['id']}.service",
            "",
            "[Install]",
            "WantedBy=timers.target",
            "",
    ...
    def sync(self) -> List[str]:
        ...
        if not self.config.is_root:
            return ["Timer definitions were saved but cannot be installed unless ServerDeck runs as root."] if enabled else []
        if not command_exists("systemctl") or not pathlib.Path("/run/systemd/system").exists():
            return ["Timer definitions were saved, but systemd is not active on this machine."] if enabled else []

        warnings: List[str] = []
        with self.lock:
            expected_ids = set(enabled)
            recurring_ids = {timer_id for timer_id, job in enabled.items() if job.get("schedule", {}).get("trigger") != "boot"}

            for timer_path in self.systemd_dir.glob("serverdeck-task-*.timer"):
                timer_id = timer_path.stem.removeprefix("serverdeck-task-")
                if timer_id not in recurring_ids:
                    run_command(["systemctl", "disable", "--now", timer_path.name], timeout=30)
                    try:
                        timer_path.unlink()
                    except FileNotFoundError:
            ...
            for service_path in self.systemd_dir.glob("serverdeck-task-*.service"):
                timer_id = service_path.stem.removeprefix("serverdeck-task-")
                if timer_id not in expected_ids:
            ...
            for timer_id, job in enabled.items():
                unit_base = f"serverdeck-task-{timer_id}"
                service_path = self.systemd_dir / f"{unit_base}.service"
                timer_path = self.systemd_dir / f"{unit_base}.timer"
                atomic_write(service_path, self._service_text(job), 0o644)
                if job.get("schedule", {}).get("trigger") == "boot":
                    run_command(["systemctl", "disable", "--now", timer_path.name], timeout=30)
                    try:
                        timer_path.unlink()
                    except FileNotFoundError:
                    ...
                    atomic_write(timer_path, self._timer_text(job), 0o644)
            ...
            for timer_id, job in enabled.items():
                unit_base = f"serverdeck-task-{timer_id}"
                if job.get("schedule", {}).get("trigger") == "boot":
                    run_command(["systemctl", "disable", f"{unit_base}.timer"], timeout=30)
                    result = run_command(["systemctl", "enable", f"{unit_base}.service"], timeout=30)
                else:
                    run_command(["systemctl", "disable", f"{unit_base}.service"], timeout=30)
                    result = run_command(["systemctl", "enable", "--now", f"{unit_base}.timer"], timeout=30)
                if result.returncode != 0:
...
def timer_log_path(config: Config, timer_id: str) -> pathlib.Path:
    return config.logs_dir / f"timer-{timer_id}.log"
...
def run_timer_sync(config: Config, store: TimerStore, timer_id: str, write_extra: Optional[Callable[[str], None]] = None) -> int:
    ...
    if not job:
        raise ValueError("Timer job not found")
    args = build_timer_args(job)
    if not args:
        raise ValueError("Timer command is empty")

    account = pwd.getpwnam(job["run_as_user"])
    if os.geteuid() != 0 and account.pw_uid != os.geteuid():
        raise PermissionError("Running a timer as another user requires root privileges")
...
def nav(active: str, username: str, csrf_token: str) -> str:
    items = [("overview", "/", "Overview"), ("updates", "/updates", "System updates"), ("backup", "/backup", "Backup"), ("timers", "/timers", "Timers")]
...
UPDATES_BODY = r"""
<section class="hero"><div><h1>System updates</h1><p>Refresh Debian or Ubuntu package information, review available upgrades, and install them in one operation.</p></div><span id="update-count" class="pill">Checking…</span></section>
...
BACKUP_BODY = r"""
<section class="hero"><div><h1>Backup jobs</h1><p>Create reusable rsync jobs, run them immediately, or schedule them with cron or systemd timers.</p></div><span id="rsync-pill" class="pill">Checking rsync…</span></section>
<div id="backup-notice" class="notice hidden"></div>
<div class="grid" style="margin-top:18px">
  <article class="card span-5"><div class="section-title"><h2 id="form-title">New backup job</h2><button class="button ghost hidden" id="cancel-edit">Cancel edit</button></div>
  ...
  <article class="card"><div class="section-title"><h2>Backup output</h2><span id="backup-state" class="pill">Idle</span></div><pre class="output" id="backup-output">Select “Run now” or “View log” on a saved job.</pre></article></section>
</div>
...
function resetForm(){document.getElementById('backup-form').reset();document.getElementById('job-id').value='';document.getElementById('form-title').textContent='New backup job';document.getElementById('cancel-edit').classList.add('hidden');document.getElementById('opt-archive').checked=true;document.getElementById('opt-itemize').checked=true;document.getElementById('opt-verbose').checked=true;document.getElementById('opt-human').checked=true;document.getElementById('opt-progress').checked=true;document.getElementById('opt-update').checked=true;backend.value='manual';preset.value='daily';document.getElementById('schedule-enabled').checked=true;document.getElementById('chmod-field').classList.add('hidden');updateScheduleFields();}
...
TIMERS_BODY = r"""
<section class="hero"><div><h1>Timers and startup tasks</h1><p>Run Python scripts or system commands at startup or on a recurring schedule, without manually creating systemd unit files.</p></div><span id="timers-pill" class="pill">Checking systemd…</span></section>
<div id="timers-notice" class="notice warning">Scheduled commands can modify or delete files, especially when run as root. Review every command and use “Run now” to test it before enabling the schedule.</div>
<div class="grid" style="margin-top:18px">
  <article class="card span-5"><div class="section-title"><h2 id="timer-form-title">New timer</h2><button class="button ghost hidden" id="timer-cancel-edit" type="button">Cancel edit</button></div>
    <form id="timer-form" class="stack">
      <input type="hidden" id="timer-id">
      <div class="field"><label for="timer-name">Task name</label><input class="input" id="timer-name" maxlength="80" required placeholder="Remove old download files"></div>
      <div class="form-grid"><div class="field"><label for="timer-kind">Task type</label><select class="select" id="timer-kind"><option value="python">Python script</option><option value="command">System command</option></select></div><div class="field"><label for="timer-user">Run as user</label><select class="select" id="timer-user"></select></div></div>
      <div id="python-fields" class="stack">
        <div class="field"><label for="timer-script">Python script</label><div class="input-with-button"><input class="input mono" id="timer-script" placeholder="/opt/scripts/import_media.py"><button class="button browse-button" type="button" data-timer-browse="file" data-target="timer-script">Browse</button></div></div>
        <div class="field"><label for="timer-interpreter">Python interpreter</label><input class="input mono" id="timer-interpreter" value="/usr/bin/python3"></div>
        <div class="field"><label for="timer-arguments">Script arguments</label><input class="input mono" id="timer-arguments" placeholder="--scan /srv/media --quiet"><div class="help">Arguments support normal command-line quoting, such as <span class="mono">--name &quot;Media Library&quot;</span>.</div></div>
      </div>
      <div id="command-fields" class="field hidden"><label for="timer-command">Command and arguments</label><textarea class="textarea mono" id="timer-command" rows="3" placeholder="find /srv/downloads -type f -mtime +30 -delete"></textarea><div class="help">Commands are executed directly, not through a shell. Pipes and redirection require an explicit command such as <span class="mono">/bin/sh -c '...'</span>.</div></div>
      <div class="field"><label for="timer-working-directory">Working directory (optional)</label><div class="input-with-button"><input class="input mono" id="timer-working-directory" placeholder="Uses the selected user's home folder"><button class="button browse-button" type="button" data-timer-browse="folder" data-target="timer-working-directory">Browse</button></div></div>
      <div class="form-grid"><div class="field"><label for="timer-trigger">Run schedule</label><select class="select" id="timer-trigger"><option value="boot">Whenever the server starts</option><option value="hourly">Hourly</option><option value="daily" selected>Daily at 02:00</option><option value="weekly">Weekly, Sunday at 02:00</option><option value="monthly">Monthly, day 1 at 02:00</option><option value="custom">Custom systemd calendar</option></select></div><div class="field" id="boot-delay-field"><label for="timer-boot-delay">Startup delay (seconds)</label><input class="input" id="timer-boot-delay" type="number" min="0" max="3600" value="30"></div></div>
      <div class="field hidden" id="timer-expression-field"><label for="timer-expression">Systemd OnCalendar expression</label><input class="input mono" id="timer-expression" placeholder="Mon..Fri *-*-* 03:30:00"><div class="help">Example: weekdays at 03:30. The server's local timezone is used.</div></div>
      <div class="checks"><label class="check"><input type="checkbox" id="timer-enabled" checked><span><strong>Enable schedule</strong><small>Install and activate the task when saved.</small></span></label><label class="check" id="timer-persistent-wrap"><input type="checkbox" id="timer-persistent" checked><span><strong>Catch up after downtime</strong><small>Run a missed calendar event after the server returns.</small></span></label></div>
      <button class="button primary" type="submit" id="timer-save"><span class="spinner"></span>Save timer</button>
    </form>
  </article>
  <section class="span-7 stack"><article class="card"><div class="section-title"><div><h2>Saved timers</h2><div class="metric-detail">Boot tasks run on the next startup. Calendar tasks use systemd timers.</div></div></div><div id="timers-list" class="stack"><div class="empty">Loading timers…</div></div></article>
  <article class="card"><div class="section-title"><h2>Task output</h2><span id="timer-state" class="pill">Idle</span></div><pre class="output" id="timer-output">Choose “Run now” or “View log” on a saved task.</pre></article></section>
</div>
<div class="modal" id="timer-browser-modal" role="dialog" aria-modal="true" aria-labelledby="timer-browser-title"><div class="modal-box browser-box"><div class="section-title"><div><h2 id="timer-browser-title">Choose path</h2><div class="metric-detail" id="timer-browser-subtitle">Items shown here are on the server.</div></div><button class="button ghost" type="button" id="timer-browser-close">Close</button></div><div class="browser-toolbar"><button class="button" type="button" id="timer-browser-up">Up one folder</button><button class="button" type="button" id="timer-browser-root">Root /</button></div><div class="browser-path mono" id="timer-browser-current">/</div><div id="timer-browser-notice" class="notice warning hidden" style="margin-bottom:12px"></div><div class="browser-list" id="timer-browser-list"><div class="browser-status">Loading…</div></div><div class="actions" style="margin-top:16px"><button class="button primary" type="button" id="timer-browser-select">Use this folder</button><button class="button ghost" type="button" id="timer-browser-cancel">Cancel</button></div></div></div>
"""

TIMERS_JS = r"""
let timerJobs=[],timerCurrentUser='root';
const timerKind=document.getElementById('timer-kind'),timerTrigger=document.getElementById('timer-trigger');
function updateTimerFields(){const python=timerKind.value==='python',boot=timerTrigger.value==='boot',custom=timerTrigger.value==='custom';document.getElementById('python-fields').classList.toggle('hidden',!python);document.getElementById('command-fields').classList.toggle('hidden',python);document.getElementById('boot-delay-field').classList.toggle('hidden',!boot);document.getElementById('timer-expression-field').classList.toggle('hidden',!custom);document.getElementById('timer-persistent-wrap').classList.toggle('hidden',boot);}
timerKind.onchange=updateTimerFields;timerTrigger.onchange=updateTimerFields;
const timerBrowserModal=document.getElementById('timer-browser-modal'),timerBrowserList=document.getElementById('timer-browser-list'),timerBrowserCurrent=document.getElementById('timer-browser-current'),timerBrowserUp=document.getElementById('timer-browser-up'),timerBrowserSelect=document.getElementById('timer-browser-select');
let timerBrowserTarget=null,timerBrowserMode='folder',timerBrowserPath='/',timerBrowserParent=null,timerSelectedFile='';
function timerBrowserStart(value){if(!value||!value.startsWith('/'))return '/';if(timerBrowserMode==='file'){const pos=value.lastIndexOf('/');return pos<=0?'/':value.slice(0,pos)}return value;}
function closeTimerBrowser(){timerBrowserModal.classList.remove('open');timerBrowserTarget=null;timerSelectedFile='';}
async function loadTimerBrowser(path){timerBrowserList.innerHTML='<div class="browser-status">Loading…</div>';timerBrowserSelect.disabled=timerBrowserMode==='file';hideNotice('timer-browser-notice');try{const data=await api('/api/filesystem?path='+encodeURIComponent(path)+(timerBrowserMode==='file'?'&files=1':''));timerBrowserPath=data.path;timerBrowserParent=data.parent;timerBrowserCurrent.textContent=data.path;timerBrowserUp.disabled=!data.parent;if(data.adjusted_from)showNotice('timer-browser-notice','Showing the nearest existing folder: '+data.path,'warning');timerBrowserList.textContent='';if(!data.entries.length){timerBrowserList.innerHTML='<div class="browser-status">No matching items are visible here.</div>';return}for(const entry of data.entries){const button=document.createElement('button');button.type='button';button.className='browser-entry';const icon=document.createElement('span');icon.className='browser-entry-icon';icon.textContent=entry.directory?'▸':'PY';const name=document.createElement('span');name.className='browser-entry-name mono';name.textContent=entry.name+(entry.symlink?' →':'');button.append(icon,name);if(entry.directory){button.onclick=()=>loadTimerBrowser(entry.path)}else{button.onclick=()=>{timerSelectedFile=entry.path;timerBrowserCurrent.textContent=entry.path;timerBrowserList.querySelectorAll('.browser-entry').forEach(item=>item.style.background='');button.style.background='var(--panel2)';timerBrowserSelect.disabled=false}}timerBrowserList.appendChild(button)}}catch(error){timerBrowserList.innerHTML='<div class="browser-status">Unable to display this location.</div>';showNotice('timer-browser-notice',error.message,'danger')}}
function openTimerBrowser(button){timerBrowserTarget=document.getElementById(button.dataset.target);timerBrowserMode=button.dataset.timerBrowse;timerSelectedFile='';document.getElementById('timer-browser-title').textContent=timerBrowserMode==='file'?'Choose Python script':'Choose working directory';document.getElementById('timer-browser-subtitle').textContent='Items shown here are on the server.';timerBrowserSelect.textContent=timerBrowserMode==='file'?'Use this file':'Use this folder';timerBrowserModal.classList.add('open');loadTimerBrowser(timerBrowserStart(timerBrowserTarget.value));}
document.querySelectorAll('[data-timer-browse]').forEach(button=>button.onclick=()=>openTimerBrowser(button));timerBrowserUp.onclick=()=>{if(timerBrowserParent)loadTimerBrowser(timerBrowserParent)};document.getElementById('timer-browser-root').onclick=()=>loadTimerBrowser('/');timerBrowserSelect.onclick=()=>{if(timerBrowserTarget){timerBrowserTarget.value=timerBrowserMode==='file'?timerSelectedFile:timerBrowserPath}closeTimerBrowser()};document.getElementById('timer-browser-close').onclick=closeTimerBrowser;document.getElementById('timer-browser-cancel').onclick=closeTimerBrowser;timerBrowserModal.addEventListener('click',event=>{if(event.target===timerBrowserModal)closeTimerBrowser()});
function timerPayload(){return {name:document.getElementById('timer-name').value,kind:timerKind.value,script:document.getElementById('timer-script').value,interpreter:document.getElementById('timer-interpreter').value,arguments:document.getElementById('timer-arguments').value,command:document.getElementById('timer-command').value,working_directory:document.getElementById('timer-working-directory').value,run_as_user:document.getElementById('timer-user').value,schedule:{enabled:document.getElementById('timer-enabled').checked,trigger:timerTrigger.value,expression:document.getElementById('timer-expression').value,persistent:document.getElementById('timer-persistent').checked,boot_delay:Number(document.getElementById('timer-boot-delay').value||0)}}}
function resetTimerForm(){document.getElementById('timer-form').reset();const userSelect=document.getElementById('timer-user');if([...userSelect.options].some(option=>option.value===timerCurrentUser))userSelect.value=timerCurrentUser;else if([...userSelect.options].some(option=>option.value==='root'))userSelect.value='root';document.getElementById('timer-id').value='';document.getElementById('timer-form-title').textContent='New timer';document.getElementById('timer-cancel-edit').classList.add('hidden');timerKind.value='python';timerTrigger.value='daily';document.getElementById('timer-interpreter').value='/usr/bin/python3';document.getElementById('timer-boot-delay').value='30';document.getElementById('timer-enabled').checked=true;document.getElementById('timer-persistent').checked=true;updateTimerFields();}
function timerScheduleText(job){const s=job.schedule;if(!s.enabled)return 'Disabled';if(s.trigger==='boot')return 'At startup · '+s.boot_delay+'s delay';const labels={hourly:'Hourly',daily:'Daily at 02:00',weekly:'Sunday at 02:00',monthly:'Day 1 at 02:00',custom:s.expression};return labels[s.trigger]||s.trigger;}
function renderTimers(){const holder=document.getElementById('timers-list');if(!timerJobs.length){holder.innerHTML='<div class="empty">No timer tasks have been created.</div>';return}holder.innerHTML=timerJobs.map(job=>'<div class="job"><div class="job-head"><div><h3>'+esc(job.name)+'</h3><div class="path mono">'+esc(job.display_command)+'</div></div><span class="pill '+(job.schedule.enabled?'good':'')+'">'+esc(timerScheduleText(job))+'</span></div><div class="job-meta"><span class="pill">Run as '+esc(job.run_as_user)+'</span>'+(job.working_directory?'<span class="pill mono">cwd '+esc(job.working_directory)+'</span>':'')+'</div><div class="job-actions"><button class="button primary" onclick="runTimer(\''+job.id+'\')">Run now</button><button class="button" onclick="editTimer(\''+job.id+'\')">Edit</button><button class="button" onclick="viewTimerLog(\''+job.id+'\')">View log</button><button class="button danger" onclick="deleteTimer(\''+job.id+'\')">Delete</button></div></div>').join('')}
async function loadTimers(){try{const data=await api('/api/timers');timerCurrentUser=data.current_user||'root';timerJobs=data.jobs.map(job=>Object.assign({},job,{display_command:job.display_command||''}));const userSelect=document.getElementById('timer-user'),previous=userSelect.value;userSelect.innerHTML=data.users.map(user=>'<option value="'+esc(user)+'">'+esc(user)+'</option>').join('');if(data.users.includes(previous))userSelect.value=previous;else if(data.users.includes(data.current_user))userSelect.value=data.current_user;else if(data.users.includes('root'))userSelect.value='root';const pill=document.getElementById('timers-pill');pill.textContent=data.capabilities.systemd_available?'systemd ready':'systemd unavailable';pill.className='pill '+(data.capabilities.systemd_available?'good':'warn');renderTimers();if(data.warnings?.length)showNotice('timers-notice',data.warnings.join(' '),'warning')}catch(error){showNotice('timers-notice',error.message,'danger')}}
window.editTimer=id=>{const job=timerJobs.find(item=>item.id===id);if(!job)return;document.getElementById('timer-id').value=job.id;document.getElementById('timer-name').value=job.name;timerKind.value=job.kind;document.getElementById('timer-script').value=job.script||'';document.getElementById('timer-interpreter').value=job.interpreter||'/usr/bin/python3';document.getElementById('timer-arguments').value=job.arguments||'';document.getElementById('timer-command').value=job.command||'';document.getElementById('timer-working-directory').value=job.working_directory||'';document.getElementById('timer-user').value=job.run_as_user;timerTrigger.value=job.schedule.trigger;document.getElementById('timer-expression').value=job.schedule.expression||'';document.getElementById('timer-boot-delay').value=job.schedule.boot_delay??30;document.getElementById('timer-enabled').checked=job.schedule.enabled;document.getElementById('timer-persistent').checked=job.schedule.persistent;document.getElementById('timer-form-title').textContent='Edit timer';document.getElementById('timer-cancel-edit').classList.remove('hidden');updateTimerFields();window.scrollTo({top:0,behavior:'smooth'})};
window.deleteTimer=async id=>{if(!confirm('Delete this task and remove its installed systemd schedule?'))return;try{const result=await api('/api/timers/'+id,{method:'DELETE'});showNotice('timers-notice','Timer deleted.'+(result.warnings?.length?' '+result.warnings.join(' '):''),result.warnings?.length?'warning':'good');await loadTimers()}catch(error){showNotice('timers-notice',error.message,'danger')}};
window.runTimer=async id=>{try{const task=await api('/api/timers/'+id+'/run',{method:'POST'});const state=document.getElementById('timer-state');state.textContent='Running';state.className='pill warn';document.getElementById('timer-output').textContent='Starting task…';pollTask(task.id,'timer-output',finished=>{state.textContent=finished.returncode===0?'Completed':'Failed';state.className='pill '+(finished.returncode===0?'good':'bad')})}catch(error){showNotice('timers-notice',error.message,'danger')}};
window.viewTimerLog=async id=>{try{const data=await api('/api/timers/'+id+'/log');const output=document.getElementById('timer-output');output.textContent=data.log;output.scrollTop=output.scrollHeight}catch(error){showNotice('timers-notice',error.message,'danger')}};
document.getElementById('timer-form').onsubmit=async event=>{event.preventDefault();const button=document.getElementById('timer-save'),id=document.getElementById('timer-id').value;setBusy(button,true);try{const result=await api('/api/timers'+(id?'/'+id:''),{method:id?'PUT':'POST',body:timerPayload()});showNotice('timers-notice','Timer saved.'+(result.warnings?.length?' '+result.warnings.join(' '):''),result.warnings?.length?'warning':'good');resetTimerForm();await loadTimers()}catch(error){showNotice('timers-notice',error.message,'danger')}finally{setBusy(button,false)}};document.getElementById('timer-cancel-edit').onclick=resetTimerForm;resetTimerForm();loadTimers();
"""
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/":
                ...
                self.send_html(page("System updates", "updates", UPDATES_BODY, UPDATES_JS, csrf, username))
            elif path == "/backup":
                self.send_html(page("Backup", "backup", BACKUP_BODY, BACKUP_JS, csrf, username))
            elif path == "/timers":
                self.send_html(page("Timers", "timers", TIMERS_BODY, TIMERS_JS, csrf, username))
            elif path == "/api/stats":
            ...
            elif re.fullmatch(r"/api/timers/[A-Za-z0-9_-]+/log", path):
                timer_id = path.split("/")[3]
                if not self.app.timer_jobs.get(timer_id):
                    self.error_json(404, "Timer job not found")
                else:
    ...
    def do_POST(self) -> None:
        ...
        try:
            if path == "/api/hostname":
            ...
            elif re.fullmatch(r"/api/timers/[A-Za-z0-9_-]+/run", path):
                timer_id = path.split("/")[3]
                ...
                task = self.app.tasks.start("timer", f"Run timer: {job['name']}", f"timer:{timer_id}", timer_worker)
                self.send_json(task, 202)
            else:
                self.error_json(404, "Not found")
        except PermissionError as exc:
    ...
    def do_PUT(self) -> None:
        ...
        timer_match = re.fullmatch(r"/api/timers/([A-Za-z0-9_-]+)", path)
...
def uninstall_service() -> int:
    ...
    print("ServerDeck service and managed schedule units removed. Backup and timer job data were left in place.")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--run-timer", metavar="TIMER_ID", help=argparse.SUPPRESS)
...
def main() -> int:
    ...
    if args.run_timer:
        ...
        try:
            return run_timer_sync(config, store, args.run_timer)
        except Exception as exc:
            print(f"Timer failed: {exc}", file=sys.stderr)
            return 1
    ...
    if not config.is_root:
        print("Running without root: hostname changes, package operations, power actions, and system schedules will be unavailable.")
