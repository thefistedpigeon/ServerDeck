...
APP_NAME = "ServerDeck"
APP_VERSION = "0.8.0"
APP_BUILD = "2026-07-30-terminal-page"
DEFAULT_PORT = 8081
MAX_BODY = 1024 * 1024
...
def nav(active: str, username: str, csrf_token: str) -> str:
    items = [("overview", "/", "Overview"), ("updates", "/updates", "Updates"), ("backup", "/backup", "rSync"), ("terminal", "/terminal", "Terminal")]
...
<div class="modal" id="power-modal" role="dialog" aria-modal="true" aria-labelledby="power-title"><div class="modal-box"><div class="section-title"><div><h2 id="power-title">Power Options</h2><div class="metric-detail">Restart or safely shut down this server.</div></div><span class="pill warn">Administrator only</span></div><div class="notice warning">Active SSH sessions, updates, and rSync jobs will be interrupted.</div><div class="actions" style="margin-top:18px"><button class="button" id="restart-btn"><span class="spinner"></span>Restart server</button><button class="button danger" id="shutdown-btn"><span class="spinner"></span>Shut down server</button><button class="button ghost" id="power-cancel">Cancel</button></div><div id="power-notice" class="notice hidden" style="margin-top:14px"></div></div></div>
"""
...
powerCancelButton.onclick=()=>powerModal.classList.remove('open');
powerModal.addEventListener('click',event=>{if(event.target===powerModal)powerModal.classList.remove('open')});
let statsTimer=null;
...
def main() -> int:
    ...
    if not config.is_root:
        print("Running without root: hostname changes, package operations, power actions, system schedules, and terminal user switching will be unavailable.")
