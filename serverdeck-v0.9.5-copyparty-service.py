...
APP_NAME = "ServerDeck"
APP_VERSION = "0.9.5"
APP_BUILD = "2026-08-11-copyparty-service"
DEFAULT_PORT = 9090
MAX_BODY = 1024 * 1024
...
class CopyPartyManager:
    ...
    def _render_service_unit(folder: pathlib.Path, username: str) -> str:
        python3 = "/usr/bin/python3" if pathlib.Path("/usr/bin/python3").exists() else (shutil.which("python3") or sys.executable)
        python_path = pathlib.Path(python3)
        if not python_path.is_absolute():
            raise RuntimeError("Unable to locate an absolute Python 3 executable")
        volume = f"{folder}::rw"
        return f"""{COPYPARTY_SERVICE_MARKER}\n[Unit]\nDescription=CopyParty file server (managed by ServerDeck)\nWants=network-online.target\nAfter=network-online.target local-fs.target\n\n[Service]\nType=simple\nUser={username}\nExecStartPre=/usr/bin/test -r {systemd_quote(str(folder))}\nExecStartPre=/usr/bin/test -w {systemd_quote(str(folder))}\nExecStartPre=/usr/bin/test -x {systemd_quote(str(folder))}\nExecStart={python_path} {COPYPARTY_SCRIPT_PATH} -v {systemd_quote(volume)} -z\nRestart=on-failure\nRestartSec=3\nNoNewPrivileges=true\nPrivateTmp=true\nProtectSystem=full\nUMask=0022\n\n[Install]\nWantedBy=multi-user.target\n"""
        ...
        service.update(
            {
                ...
                "command": f"copyparty-sfx.py -v {configured_folder}::rw -z" if configured_folder else "",
    ...
    def configure_and_start(self, payload: Dict[str, Any], username: str) -> Dict[str, Any]:
        ...
        if not COPYPARTY_SCRIPT_PATH.is_file():
            raise RuntimeError("Download CopyParty before configuring its service")
        if not command_exists("systemctl"):
        ...
        return {"message": f"CopyParty is configured, running, and enabled at boot using {folder}.", "service": self.snapshot()["service"]}
...
<div class="grid">
  ...
  <article class="card span-12">
    <div class="section-title"><div><h2>Configure and run</h2><div class="metric-detail">Creates <span class="mono">copyparty.service</span>, enables it at boot, and starts it immediately.</div></div><span id="copyparty-service-pill" class="pill">Not configured</span></div>
    <div class="form-grid">
      ...
      <div class="field full"><label>Service command</label><pre class="output" id="copyparty-command" style="min-height:72px;max-height:110px">copyparty-sfx.py -v /path/to/folder::rw -z</pre></div>
    </div>
...
function updateCopyPartyCommand(){const folder=cpFolder.value.trim()||'/path/to/folder';document.getElementById('copyparty-command').textContent='copyparty-sfx.py -v '+JSON.stringify(folder+'::rw')+' -z';}
...
async function loadCopyParty(){
  try{
    ...
    document.getElementById('copyparty-start').disabled=!svc.configured||svc.active;document.getElementById('copyparty-restart').disabled=!svc.configured;document.getElementById('copyparty-stop').disabled=!svc.configured||!svc.active;
